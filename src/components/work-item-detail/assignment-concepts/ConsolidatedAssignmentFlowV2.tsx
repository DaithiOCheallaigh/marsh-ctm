        ...
        <Badge 
          ...
          className={cn(
            ...
            chair.type === 'primary' 
              ...
              : isSelected ? "border-primary-foreground/50 text-primary-foreground" : "bg-gray-50 text-gray-600 border-gray-200"
          )}
        >
...
export const ConsolidatedAssignmentFlowV2 = ({
  ...
  const getAssignedChairIds = (): Set<string> => {
    ...
    existingAssignments
      .filter(a => a.roleId === selectedRole?.roleId)
      .forEach(a => {
  ...
  const getMemberAvailableCapacity = (member: TeamMember): number => {
    ...
    const pendingWorkload = pendingAssignments
      .filter(p => p.member.id === member.id)
      .reduce((sum, p) => sum + p.workloadPercentage, 0);
  ...
  const eligibleMembers = useMemo(() => {
    ...
    const membersWithStatus = members.map(m => ({
      ...
      disableReason: assignedMemberIds.has(m.id) 
        ? "Already assigned to a chair in this role" 
        : undefined
    }));
  ...
  const allChairs = useMemo(() => {
    ...
    return SAMPLE_CHAIRS.map(chair => ({
      ...
      disableReason: assignedChairIds.has(chair.id) 
        ? "Already assigned in this role" 
        : chair.assignedTo 
          ? `Assigned to ${chair.assignedTo}`
          : undefined
    }));
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between pt-4 border-t">
                ...
                <Button 
                  ...
                  disabled={isReadOnly || pendingAssignments.length === 0}
                >
              ...
              <div className="bg-muted/30 rounded-lg p-4">
                ...
                <div className="space-y-3">
                  {pendingAssignments.map((assignment, index) => (
                    <div 
                      key={assignment.id}
                      className="bg-white rounded-lg p-4 border border-[hsl(var(--wq-border))]"
                    >
