...
export const EnhancedAssignmentPanel: React.FC<EnhancedAssignmentPanelProps> = ({
  ...
  const assignedCapacity = useMemo(() => {
    return clients
      ...
      .reduce((sum, c) => sum + (c.capacityRequirement || 1), 0);
  }, [clients, assignedClientIds]);
  ...
  const selectedClientsCapacity = useMemo(() => {
    return clients
      ...
      .reduce((sum, c) => sum + (c.capacityRequirement || 1) * 20, 0); // 20% per chair
  ...
  return (
    <div className="space-y-4">
      {/* Summary Bar */}
      <div className="flex items-center justify-between p-3 bg-[hsl(var(--wq-bg-header))] rounded-lg border border-[hsl(var(--wq-border))]">
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2">
            <Briefcase className="w-4 h-4 text-[hsl(var(--wq-text-muted))]" />
            <span className="text-xs text-[hsl(var(--wq-text-secondary))]">
              Remaining to Assign:
            </span>
            <span className="text-sm font-semibold text-primary">
              {remainingCount} clients | {remainingCapacity.toFixed(1)} capacity
            </span>
          </div>
          <div className="h-4 w-px bg-[hsl(var(--wq-border))]" />
          <div className="flex items-center gap-2">
            <span className="text-xs text-[hsl(var(--wq-text-secondary))]">Assigned:</span>
            <span className="text-sm font-semibold text-[hsl(var(--wq-status-completed-text))]">
              {assignedCount} clients | {assignedCapacity.toFixed(1)} capacity
            </span>
          </div>
        </div>
        {/* Progress indicator */}
        <div className="flex items-center gap-2">
          <div className="w-32 h-2 bg-[hsl(var(--wq-bg-muted))] rounded-full overflow-hidden">
            <div
              className="h-full bg-[hsl(var(--wq-status-completed-text))] transition-all duration-300"
              style={{ width: `${clients.length > 0 ? (assignedCount / clients.length) * 100 : 0}%` }}
            />
          </div>
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-6">
        ...
        <div className="space-y-4">
          ...
          {remainingCount > 0 && assignedCount > 0 && (
            <div className="flex items-center gap-2 px-3 py-2 bg-[hsl(var(--wq-priority-high))]/10 border border-[hsl(var(--wq-priority-high))]/30 rounded-lg">
          ...
          <div className="space-y-2 max-h-[400px] overflow-auto">
            {availableClients.map((client) => (
              <LeaverClientCard
                ...
                capacityRequirement={client.capacityRequirement}
              />
            ))}
            {availableClients.length === 0 && (
              <div className="flex items-center justify-center py-8 text-[hsl(var(--wq-status-completed-text))]">
              ...
              {pendingClients.map((client) => (
                <div
                  ...
                  className={cn(
                    ...
                    pendingSelectedClientIds.includes(client.id)
                ...
                >
                  ...
                  {client.capacityRequirement && (
                    <span className="text-xs text-[hsl(var(--wq-text-muted))]">
