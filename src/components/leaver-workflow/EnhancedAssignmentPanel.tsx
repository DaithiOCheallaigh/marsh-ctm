...
export const EnhancedAssignmentPanel: React.FC<EnhancedAssignmentPanelProps> = ({
    ...
    () =>
      clients.filter(
        (client) =>
          ...
          !pendingClientIds.includes(client.id)
      ),
    [clients, assignedClientIds, pendingClientIds]
  );
  ...
  const selectedClientsCapacity = useMemo(() => {
    return clients
      ...
      .reduce((sum, c) => sum + (c.capacityRequirement || 1) * 20, 0);
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2 max-h-[400px] overflow-auto">
            ...
            {availableClients.length === 0 && pendingClients.length === 0 && (
            ...
            {availableClients.length === 0 && pendingClients.length > 0 && (
              <div className="flex items-center justify-center py-8 text-[hsl(var(--wq-text-muted))]">
        ...
        <div className="flex flex-col items-center justify-center gap-2 py-8">
          <button
            ...
            aria-label="Move selected clients to pending"
          >
            <ArrowRight className="w-5 h-5" />
          </button>
          <button
            onClick={onRemoveFromPending}
            disabled={!canRemoveFromPending}
            className={cn(
              "w-10 h-10 rounded-lg flex items-center justify-center transition-all",
            ...
            aria-label="Remove selected clients from pending"
          >
