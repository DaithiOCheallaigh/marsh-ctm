...
const TeamCard = ({
  ...
  const handleRoleToggle = (role: RoleWithMaxChairs, checked: boolean) => {
    ...
    if (checked) {
      ...
      updatedRoles = [
        ...config.roles,
        { roleId: role.id, roleName: role.roleName, chairRequirement: 1 }
      ];
    } else {
      // Remove role
  ...
  const handleChairRequirementChange = (roleId: string, value: number) => {
    const updatedRoles = config.roles.map(r => 
      r.roleId === roleId ? { ...r, chairRequirement: Math.max(1, Math.min(10, value)) } : r
    );
    onUpdate({ roles: updatedRoles });
  };

  const isRoleSelected = (roleId: string) => 
    config.roles.some(r => r.roleId === roleId);

  const getRoleChairRequirement = (roleId: string) => 
    config.roles.find(r => r.roleId === roleId)?.chairRequirement || 1;
                    ...
                    {isSelected && (
                      <div className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          max={10}
                          value={chairValue}
                          onChange={(e) => handleChairRequirementChange(role.id, parseInt(e.target.value) || 1)}
                          className={cn(
                            "w-16 h-8 px-2 text-sm rounded-md border text-center",
