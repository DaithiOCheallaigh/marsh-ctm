...
const LeaverFields = ({
  ...
  return <div className="bg-white rounded-lg border border-border-primary p-6 space-y-6">
      ...
      <div className="grid grid-cols-[180px_1fr] items-center gap-4">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" className={cn("max-w-md justify-start text-left font-normal border-border-primary", !leavingDate && "text-muted-foreground")}>
              {leavingDate ? format(leavingDate, "MM/dd/yyyy") : "Select date"}
              <CalendarIcon className="ml-auto h-4 w-4 opacity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-auto p-0 bg-white z-50" align="start">
            <Calendar mode="single" selected={leavingDate} onSelect={date => {
            setLeavingDate(date);
      ...
      <div className="grid grid-cols-[180px_1fr] items-center gap-4">
        ...
        <div className="space-y-1">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("max-w-md justify-start text-left font-normal border-border-primary", !dueDate && "text-muted-foreground")}>
