...
import { leaverClients, LeaverClient, Reassignment } from "@/data/leaverClients";
import { EnhancedAssignmentPanel, LeaverTeamMember, CompleteReassignmentModal, UnsavedChangesModal, CapacityWarningModal } from "@/components/leaver-workflow";
...
const LeaverWorkflow = () => {
  ...
  const handleTogglePendingClient = useCallback((clientId: string) => {
    setPendingSelectedClientIds(prev => prev.includes(clientId) ? prev.filter(id => id !== clientId) : [...prev, clientId]);
  }, []);
  ...
  const handleAssign = useCallback(() => {
    ...
    const clientsToAssign = clientsWithCapacity.filter(client => selectedClientIds.includes(client.id));

    // Calculate capacity impact
    const additionalCapacity = clientsToAssign.reduce((sum, c) => sum + (c.capacityRequirement || 1) * 20, 0);
  ...
  const executeAssignment = useCallback((clientsToAssign: (LeaverClient & {
    capacityRequirement?: number;
  })[], member: LeaverTeamMember) => {
  ...
  const handleUnassign = useCallback(() => {
    ...
    setReassignments(prev => prev.filter(r => !pendingSelectedClientIds.includes(r.clientId)));

    // Remove from assigned clients
    setAssignedClientIds(prev => prev.filter(id => !pendingSelectedClientIds.includes(id)));

    // Remove from pending clients
    setPendingClients(prev => prev.filter(client => !pendingSelectedClientIds.includes(client.id)));
  ...
  const handleRemoveReassignment = useCallback((reassignmentId: string) => {
    ...
    // Remove from assigned clients
    setAssignedClientIds(prev => prev.filter(id => id !== reassignment.clientId));
    ...
    if (selectedMember && reassignment.reassignedToId === selectedMember.id) {
      setPendingClients(prev => prev.filter(client => client.id !== reassignment.clientId));
    }
    toast({
  ...
  return <>
        ...
        <div className="flex flex-1 px-6 pb-6 gap-6">
          ...
          <main className="flex-1 ml-[280px] overflow-auto">
            ...
            <nav className="flex items-center gap-1.5 text-sm mb-2" aria-label="Breadcrumb">
              <Link to="/" onClick={handleBreadcrumbClick} className="text-[hsl(var(--wq-text-secondary))] hover:text-primary transition-colors">
                Work Queue
              </Link>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className={cn("px-3 py-1.5 rounded-full text-xs font-medium border flex items-center gap-1.5", workItem.status === "Pending" ? "bg-[hsl(var(--wq-status-pending-bg))] text-[hsl(var(--wq-status-pending-text))] border-[hsl(var(--wq-status-pending-text))]" : "bg-[hsl(var(--wq-status-completed-bg))] text-[hsl(var(--wq-status-completed-text))] border-[hsl(var(--wq-status-completed-text))]")}>
            ...
            {/* Read-only banner */}
            {isReadOnly && <div className="flex items-center justify-between p-3 mb-4 bg-[hsl(var(--wq-bg-muted))] border border-[hsl(var(--wq-border))] rounded-lg">
            ...
            <Collapsible open={workDetailsOpen} onOpenChange={setWorkDetailsOpen} className="mb-4">
              <div className="bg-white rounded-lg border border-[hsl(var(--wq-border))] overflow-hidden">
                <CollapsibleTrigger className="w-full">
                  <div className="flex items-center justify-between px-6 py-4 bg-[hsl(var(--wq-bg-header))] cursor-pointer hover:bg-[#E5EEFF] transition-colors">
                    ...
                    <ChevronDown className={cn("w-5 h-5 text-primary transition-transform duration-200", workDetailsOpen && "rotate-180")} />
                  </div>
                </CollapsibleTrigger>
                <CollapsibleContent>
            ...
            {isReadOnly && <div className="grid grid-cols-3 gap-4 mb-4">
            ...
            {!isReadOnly && <Collapsible open={assignmentDetailsOpen} onOpenChange={setAssignmentDetailsOpen} className="mb-4">
                
              </Collapsible>}

            {/* Reassignments Table */}
            <div className="mb-6">
              <EnhancedReassignmentsTable reassignments={reassignments} onRemoveReassignment={isReadOnly ? undefined : handleRemoveReassignment} isReadOnly={isReadOnly} clients={clientsWithCapacity} />
            </div>

            {/* Footer Actions */}
            {!isReadOnly ? <div className="flex items-center justify-between">
                {/* Validation message */}
                {reassignments.length > 0 && !allClientsAssigned && <div className="text-sm text-destructive">
                    ⚠ {remainingClients} client{remainingClients > 1 ? "s" : ""} remaining
                    - all must be reassigned
                  </div>}
                {reassignments.length > 0 && allClientsAssigned && <div className="text-sm text-[hsl(var(--wq-status-completed-text))]">
                ...
                <div className="flex gap-4">
                  ...
                  <Button onClick={handleComplete} disabled={!allClientsAssigned || reassignments.length === 0} className="bg-primary hover:bg-primary/90">
                    Complete
                  </Button>
                </div>
      ...
      {/* Modals */}
      <CompleteReassignmentModal isOpen={showCompleteModal} onClose={() => setShowCompleteModal(false)} onConfirm={confirmComplete} totalClients={reassignments.length} teamMembersAffected={teamMembersAffected} leaverName={leaverName} />
      <UnsavedChangesModal isOpen={showUnsavedModal} onClose={() => setShowUnsavedModal(false)} onSaveAndExit={handleSaveAndExit} onExitWithoutSaving={handleExitWithoutSaving} pendingCount={reassignments.length} />
      {pendingAssignmentData && <CapacityWarningModal isOpen={showCapacityWarning} onClose={() => {
      setShowCapacityWarning(false);
      setPendingAssignmentData(null);
    }} onProceed={handleCapacityWarningProceed} teamMemberName={pendingAssignmentData.member.name} projectedCapacity={pendingAssignmentData.projectedCapacity} />}
    </>;
};
